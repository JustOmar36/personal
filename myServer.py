...
# binary
binary_type_files = set(["jpg", "jpeg", "PNG", "png", "mp3"])
...
class ResponseBuilder:
    ...
    the response design pattern to assist you in forming a response. An
    ...
    def build(self) -> bytes:
        ...
        for i in self.headers:
            response+=f"{i}{NEWLINE}"
        response+=NEWLINE #additional blank line in between headers and content
...
class HTTPServer:
    ...
    def get_request(self, request: Request) -> Response:
        ...
        # Check if the file_path is a redirect URL
        if file_path.startswith("redirect"):
            search_item = file_path.split("=")[1].split("&")[0]
            source = file_path.split("=")[2]

            for i in source_options:
                if(i == source):
                    redirect_url = source_options[source]
            
            redirect_url += search_item
        ...
        # Check if the file has 'other' read permission
        if not has_permission_other(file_path):
            response = ResponseBuilder()
            response.set_status(403, "Forbidden")
            response.set_content("403 Forbidden")
            return response.build()

        else:
            # Read the file content and determine its MIME type
            file_extension = file_path.split(".")[1]
            should_return_binary_content = should_return_binary(file_extension)
            file_content = get_file_binary_contents(file_path) if should_return_binary_content else get_file_contents(file_path)
            ...
            response.add_header("Content-Length", len(file_content))
    ...
    def post_request(self, request: Request) -> Response:
        ...
        form_content=unquote_plus(request.content).split("&")
...
<html lang="en">
  ...
  <body>
    ...
    def resource_not_found(self) -> Response:
        ...
        response.add_header("Connection", "close")
        response.add_header("Content-Length",len(get_file_contents("404.html")))
